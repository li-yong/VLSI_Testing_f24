...
class Gate:
    def __init__(self, gate_net_id, gate_id, gate_type, gate_fanout_cnt, gate_fanin_cnt, gate_faults, input_gates_netid_list,fanout_branch_from_netid):
        ...
        self.fanout_branch_from_netid = fanout_branch_from_netid
    

    def __repr__(self):
        return f"Net ID::{self.gate_net_id}, Gate ID: {self.gate_id}, Type: {self.gate_type}, InputsCnt: {self.gate_fanin_cnt}, OutputsCnt: {self.gate_fanout_cnt}, Faults: {self.gate_faults}"
...
def parse_netlist(content):
    ...
    while line_cursor+1 < len(lines):
        ...
        if gate_id.find('gat')>0 and gate_type in ('inpt', 'nand', 'nor', 'not', 'buff', 'and', 'or', 'xor', 'xnor'):
            ...
            gate = Gate(gate_net_id, gate_id, gate_type, gate_fanout_cnt, gate_fanin_cnt, gate_faults, input_gates_netid_list,fanout_branch_from_netid)
            gates.append(gate)
