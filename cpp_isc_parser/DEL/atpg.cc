...
void CIRCUIT::Atpg()
{
    ...
    int  i, total_backtrack_num(0), pattern_num(0);
    ATPG_STATUS status;
    FAULT* fptr;
    ...
    int  total_num(0);
    int  abort_num(0), redundant_num(0), detected_num(0);
    int  eqv_abort_num(0), eqv_redundant_num(0), eqv_detected_num(0);
    for (fite = Flist.begin();fite!=Flist.end();++fite) {
        fptr = *fite;
...
//FALSE: abort
ATPG_STATUS CIRCUIT::Podem(FAULT* fptr, int  &total_backtrack_num)
{
    int  i, backtrack_num(0);
...
ATPG_STATUS CIRCUIT::SetUniqueImpliedValue(FAULT* fptr)
{
    ...
    for (int  i = 0;i < ogptr->No_Fanin();++i) {
...
void CIRCUIT::MarkPropagateTree(GATEPTR gptr)
{
    ...
    gptr->SetFlag(MARKED);
    for (int  i = 0;i<gptr->No_Fanout();++i) {
