...
void CIRCUIT::FaultSimVectors()
{
    ...
    int  total_num(0);
    int  undetected_num(0), detected_num(0);
    int  eqv_undetected_num(0), eqv_detected_num(0);
    FAULT* fptr;
...
int  CIRCUIT::FaultSimRandomPattern()
{
    ...
    total_num = detected_num + undetected_num;
		int  cov = 100*detected_num/double(total_num);
...
void CIRCUIT::InjectFaultValue(GATEPTR gptr, int  idx,VALUE value)
{
    if (value == S1) {
        gptr->SetValue1(idx);
        gptr->SetValue2(idx);
